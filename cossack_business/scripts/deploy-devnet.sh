...
echo "=== Cossack Business (Ecosystem) — Devnet Deploy ==="
echo ""
...
# The six programs that make up the game ecosystem
...
for prog in "${ALL_PROGRAMS[@]}"; do
  ...
  anchor deploy --program-name "$prog" --provider.cluster devnet || {
    ...
    anchor upgrade "target/deploy/${prog}.so" --program-id $(grep "^${prog}" Anchor.toml | head -1 | awk -F'"' '{print $2}') --provider.cluster devnet || echo "   ⚠ ${prog} upgrade also failed"
  }
done

echo ""
echo "Step 5: Run ecosystem initialization script"
